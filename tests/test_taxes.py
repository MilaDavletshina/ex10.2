...
def test_calculate_tax_invalid_price_below_zero():
    with pytest.raises(ValueError):
        calculate_tax(-1, 10)
...
def test_calculate_tax_wrong_type(price, tax_rate, discount, round_digits):
    with pytest.raises(TypeError):
        calculate_tax(price, tax_rate, discount=discount, round_digits=round_digits)  #через = написаноБ потомучто в функции указаны именованные аргументы
